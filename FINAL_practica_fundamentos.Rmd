...
```{r}
# Smaill multiple de las variables cuantitativas sin transformar
...
Para esta sección nos hemos planteado primero una serie de preguntas interesantes que puedan ser respondidas a través de visualizaciónes:

* *¿Dónde se encuentra el mayor número de nuestra población respecto de su income?* vemos que la mayoría ronda por debajo de los 500,000 $ por familia al año

```{r message=FALSE, warning=FALSE}

hist(datos_training$Total.Household.Income)
comun <- datos_training %>%
  filter(Total.Household.Income < 500000)

```

* *Cuál es la relación entre el consumo de comida y ropa respecto al income familiar?* Decidimos además filtrar por regiones y visualizar la cantidad el income familiar por el tamaño de la burbuja

```{r}

# bubble plot para ver la relación entre consumo de comida, ropa respecto al income (tamaño de la burbuja) dividido por regiones(ciudades)

datos_training %>%
  ggplot(aes(x=Clothing..Footwear.and.Other.Wear.Expenditure, y=Total.Food.Expenditure, size = Total.Household.Income, color = Region)) +
    geom_point(alpha=0.5) +
    scale_size(range = c(.1, 24), name="Income")

```


* *¿Cuáles son las profesiones más comunes de una familia Filipina?* como indicábamos al principio de esta sección, vemos que la mayoría es una población agraria 
...
* *¿Cuáles son las regiones de Filipinas con más gastos?* Nuestros datos tienen un gran número de valores atípicos, por lo que para ver el boxplot es necesario aplicar un logaritmo en base 10. Vemos que las familias de la Región `NAT` son aquellas que más gastos tienen
