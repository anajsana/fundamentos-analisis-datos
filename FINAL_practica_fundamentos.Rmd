...
```{r}
# Se realiza la regresión lineal múltiple con las variables que previamente han tenido un valor de correlación superior a 0.5
...
# En la siguiente línea calcularemos los valores "predichos"= ajustados pero con los datos de training
# Esto puedo no tener sentido, pero se hace en muchos sitios observar que los residuos tienden a la normalidad
valores.ajustados <- fitted(RLM)
# Verificamos que los valores predichos y los residuos no tienen realción lineal
plot(valores.ajustados, residuos)
# Obtenemos los betas estimados en la regresión lineal múltiple
summary(RLM)

```

```{r}
# Para comprobar que el resultado es correcto observamos en la siguiente gráfica si los residuos siguen una distribución normal. Para ello representamos el gráfico Q-Q que compara los cuantiles teóricos de una normal con los calculados. Cuantos más puntos caigan en la recta mejor
qqnorm(residuos)
qqline(residuos)
```
...
```{r}
# Del conjunto de test se seleccionan las variables necesarias
pre_datos_testing<-datos_testing%>%select(Transportation.Expenditure
          ,Clothing..Footwear.and.Other.Wear.Expenditure
        ,Fruit.Expenditure)
# Es necesario transformar logarítmicamente el conjunto de test antes de usarlo para validar
pre_datos_testing<-log(pre_datos_testing)
# Quitamos posibles valores inf
pre_datos_testing <- replace(pre_datos_testing,pre_datos_testing=="-Inf",0) 
# Realizamos la predicción con nuestro modelo de RLM calculado
ic <- predict(RLM,pre_datos_testing)
# Obtenermos en un vector los valores reales para compararlos con los predichos. Para ello, calcularemos sus residuos
Valores_reales<-log(datos_testing$Total.Household.Income)

Valores_predichos<-ic
# Calculamos los residuos
residuos<-Valores_reales-Valores_predichos
# Comprobamos que no existe ninguna relación lineal entre los valores predichos por el modelo y los residuos hallados
